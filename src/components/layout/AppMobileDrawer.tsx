...
import { Drawer, Box, Typography, IconButton, Divider, List, ListItem, ListItemButton, ListItemText, ListItemIcon, Button, InputBase, Avatar, CircularProgress } from '@mui/material';
import {
   Close as CloseIcon,
   ...
   return (
      <Drawer
         ...
         sx={{
            ...
            '& .MuiDrawer-paper': { boxSizing: 'border-box', width: { xs: '84vw', sm: 310 }, maxWidth: 320 },
         }}>
         <Box sx={{ p: 2.5, height: '100%', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            ...
            {/* User profile card or login prompt with no-flicker loading */}
            {isLoading ? (
               <Box sx={{ p: 2, mb: 2, display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 64 }}>
               ...
               <Box
                  ...
                  <Avatar src={user.avatar} alt={user.name} sx={{ width: 40, height: 40, bgcolor: 'primary.main', fontWeight: 700 }}>
                     {user.name.charAt(0)}
                  </Avatar>
            ...
            {isAuthenticated && (
               <>
                  ...
                  <List sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, p: 0 }}>
                     ...
                     <ListItem disablePadding>
                        <ListItemButton onClick={handleLogout} sx={{ borderRadius: 2, py: 0.85, px: 1.5, color: 'error.main' }}>
