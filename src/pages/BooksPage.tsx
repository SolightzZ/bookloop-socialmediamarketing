...
export default function BooksPage() {
  ...
  const [priceRange, setPriceRange] = useState<number>(
    maxPriceParam ? Number(maxPriceParam) : 2000
  );
          ...
          <Grid size={{ xs: 12, md: 9 }}>
            ...
            >
              <Box
                ...
                sx={{ display: 'flex', flexGrow: 1, maxWidth: { xs: '100%', sm: 400 } }}
              >
                <TextField
                ...
                />
                <Button type="submit" variant="contained" sx={{ ml: 1, px: 2.5 }}>
                  ค้นหา
                </Button>
              </Box>
            ...
            {filteredBooks.length > 0 && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2.5, px: 0.5 }}>
                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.9rem' }}>
                  แสดง <strong>{(validPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(validPage * ITEMS_PER_PAGE, filteredBooks.length)}</strong> จากทั้งหมด <strong>{filteredBooks.length}</strong> เล่ม (หน้า {validPage}/{totalPages})
                </Typography>
                {totalPages > 1 && (
                  <Typography variant="caption" sx={{ color: 'primary.main', fontWeight: 700, bgcolor: 'rgba(23, 105, 170, 0.08)', px: 1.5, py: 0.5, borderRadius: 1.5 }}>
                    แสดงหน้าละ 9 เล่ม
                  </Typography>
                )}
              </Box>
